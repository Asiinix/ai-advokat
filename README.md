...
Каждая партиция получает независимый `SourceClient`: свой cookie jar, свой вход в PRG.SOT и, при наличии `proxy_env`, свой HTTP(S)-прокси. Прямая партиция явно отключает ambient `http_proxy`/`https_proxy`, а явный прокси партиции игнорирует ambient `NO_PROXY`/`no_proxy` — случайная переменная контейнера не может ни завернуть трафик в чужой прокси, ни тихо пустить его мимо своего. Запросы держатся одной партиции, пока источник не сообщит, что ее квота исчерпана — HTTP 429 (в том числе на логине) или успешный ответ с `remaining=0`. Тогда партиция отдыхает ровно до собственного reset (или 60 секунд, если reset не назван; более короткая отметка никогда не сокращает уже записанный отдых), а пул прозрачно продолжает на следующей включенной. Сбой собственного egress-пути партиции — сетевая ошибка, HTTP 407 от прокси или временный HTTP 5xx при входе — отправляет ее в карантин на 300 секунд с тем же переключением; отклоненные учетные данные и обычные ошибки контента (404, 500, неверный JSON) наружу проходят без ротации.

Когда отдыхают все, обычная CLI-команда получает агрегированный HTTP 429 с самым ранним reset. Production supervisor держит тот же pool в памяти: ждет ближайшую партицию, не обращается к остальным до окончания их индивидуального `Retry-After` и продолжает тот же scan. Длинное ожидание разбивается на heartbeat-интервалы не более 300 секунд; это дает регулярный лог, но не сокращает реальный cooldown и не создает ранних запросов. Лизы, resume и защита от дублей в Postgres/SQLite не меняются: пул работает ниже уровня очереди решений. `sot-status` и `sot-probe-auth` показывают партиции (id, имя переменной прокси, состояние квоты); `sot-probe-auth` дополнительно логинит каждую партицию, подтверждая каждый egress-путь.
...
же `--scan-id`. Многопартиционный pool остается внутри одного запуска и сам
ждет ближайший доступный egress, сохраняя отдельные cooldown каждого IP. Если
временная квота или сетевой сбой возникли до создания pool/скана, supervisor
...
`FALLBACK_SECONDS` применяется только когда внешний typed-error не содержит
времени reset; `MAX_WAIT_SECONDS` ограничивает одну паузу внешнего supervisor
(по умолчанию восемь суток). Внутри живого egress-pool реальный reset не
сокращается: ожидание лишь делится на 300-секундные heartbeat-интервалы.
